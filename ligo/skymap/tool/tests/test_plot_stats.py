...
def test_plot_stats(tmpdir, groupby, ndatasets):
    ...
    for filename in filenames:
        ...
        ).write(filename, format="ascii.tab")

    args = ["ligo-skymap-plot-stats", "--format", "png", "-o", str(tmpdir)] + filenames
...
def test_plot_stats_missing(tmpdir):
    ...
    args = ["ligo-skymap-plot-stats", "--group-by", "snr", "-o", str(tmpdir), filename]
...
def test_plot_stats_invalid(tmpdir):
    ...
    Table(
        {"p_value": np.random.uniform(0, 1, size=n), "snr": np.tile(np.nan, n)}
    ).write(filename, format="ascii.tab")

    args = ["ligo-skymap-plot-stats", "--group-by", "snr", "-o", str(tmpdir), filename]
    with pytest.raises(RuntimeError) as excinfo:
        run_entry_point(*args)
    assert 'The following files had invalid values in the "snr" column' in str(
        excinfo.value
    )
