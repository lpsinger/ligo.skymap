#
# Copyright (C) 2011-2026  Will M. Farr <will.farr@ligo.org>
...
def main(args=None):
    ...
    with _parser.parse_args(args) as opts:
        ...
        if opts.enable_dpgmm:
            if opts.enable_distance_map:
        ...
        try:
            data = io.read_samples(
                opts.samples.name, path=opts.path, tablename=opts.tablename
            )
        except OSError:
            # FIXME: remove this code path once we support only HDF5
            data = Table.read(opts.samples, format="ascii")
            _remap_colnames(data)

        stopwatch = Stopwatch()
        stopwatch.start()
        if opts.maxpts is not None and opts.maxpts < len(data):
            log.info("taking random subsample of chain")
            data = data[np.random.choice(len(data), opts.maxpts, replace=False)]
        try:
            dist = data["dist"]
        except KeyError:
            try:
        ...
        if opts.loadpost is None:
            if dist is None:
                if opts.enable_distance_map:
                    _parser.error(
                        f"The posterior samples file '{opts.samples.name}' does not "
            ...
            skypost = cls(pts, trials=opts.trials, jobs=opts.jobs)

            log.info("pickling")
            with open(os.path.join(opts.outdir, "skypost.obj"), "wb") as out:
                pickle.dump(skypost, out)
        else:
            skypost = pickle.load(opts.loadpost)
            skypost.jobs = opts.jobs

        log.info("making skymap")
        hpmap = skypost.as_healpix(top_nside=opts.top_nside, rounds=opts.rounds)
        stopwatch.stop()
        ...
        if opts.objid is not None:
            hpmap.meta["objid"] = opts.objid
        if opts.instruments:
            hpmap.meta["instruments"] = opts.instruments
        if opts.enable_distance_map:
        ...
        io.write_sky_map(os.path.join(opts.outdir, opts.fitsoutname), hpmap, nest=True)
