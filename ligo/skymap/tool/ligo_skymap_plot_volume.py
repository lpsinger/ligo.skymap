#
# Copyright (C) 2015-2026  Leo Singer
#
...
def main(args=None):
    with parser().parse_args(args) as opts:
        ...
        if opts.align_to is None or opts.input.name == opts.align_to.name:
            ...
            skymap2 = io.read_sky_map(opts.align_to.name, distances=True, moc=True)
        ...
        gs = gridspec.GridSpec(
            n, n, left=0.01, right=0.99, bottom=0.01, top=0.99, wspace=0.05, hspace=0.05
        )
        ...
        for iface, (axis0, axis1, (sp0, sp1)) in enumerate(
            ...
            density = volume_render(
                xx.ravel(), yy.ravel(), max_distance, axis0, axis1, rot, skymap
            ).reshape(xx.shape)
        ...
        if not opts.projection:
            ...
            dp_dd = marginal_pdf(
                d,
                moc.uniq2pixarea(skymap["UNIQ"]) * skymap["PROBDENSITY"],
                skymap["DISTMU"],
                skymap["DISTSIGMA"],
                skymap["DISTNORM"],
            )
            ax.fill_between(d, dp_dd, alpha=0.5, color=ax._get_lines.get_next_color())
            ...
            if opts.radecdist:
                ...
                for ra, dec, dist in opts.radecdist:
                    match_ipix = hp.ang2pix(
                        HEALPIX_MACHINE_NSIDE, ra, dec, nest=True, lonlat=True
                    )
                    i = sorter[
                        np.searchsorted(index, match_ipix, side="right", sorter=sorter)
                        - 1
                    ]
                    (lines,) = ax.plot(
                        [dist],
                    ...
                    ax.fill_between(d, dp_dd, alpha=0.5, color=lines.get_color())
